...
fun get_babies_data(babies_data, search_list, offsetSt, num_entries) =
  ...
  let
    fun get_babies_info(babies_data, search_list) = 
      
      let 
        fun get_babies_trimmed(trimmed_data, search_list) = 

          let 
            fun get_stats(list) = 
              let 
                fun stats(list, pos:int, years:int) = 
                  if size_of_list(tl(list)) = 1
                  then 
                    let 
                      fun output_stats(pos, years) = 
                        let 
                    ...
                    in 
                      if 0 < valOf(fromString(hd(list)))
                      then output_stats(pos+1, years+1)
                      else output_stats(pos+1, years)
                    end 
                  else if 0 < valOf(fromString(hd(list)))
                  then stats(tl(list), pos+1, years+1)
                  else stats(tl(list), pos+1, years)
              in 
                stats(list, 0, 0)
              end
          ...
          in
            ...
            then "Baby name ["^ hd(search_list) ^"] was not found\n"^ get_babies_info(babies_data, tl(search_list))
            else if hd(search_list) = hd(hd(trimmed_data))
            then hd(search_list) ^ "\n" ^ get_stats(tl(hd(trimmed_data))) ^ get_babies_info(babies_data, tl(search_list))
            else get_babies_trimmed(tl(trimmed_data), search_list)
          end
      ...
      in
        ...
        then ""
        else get_babies_trimmed(babies_data, search_list)
      end
  in
    get_babies_info(babies_data, search_list)
  end 
